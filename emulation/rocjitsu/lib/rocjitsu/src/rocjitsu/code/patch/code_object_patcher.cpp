...
[[nodiscard]] bool relocate_relative_text_addends(std::vector<uint8_t> &image,
                                                  const Elf64_Ehdr &ehdr,
                                                  std::span<const Elf64_Shdr> shdrs,
                                                  size_t text_index, uint64_t old_text_size,
                                                  const TextPlacementIndex &placements) {
  ...
  for (const Elf64_Shdr &relocs : shdrs) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      if (placements.conflicting_sources.contains(source_offset))
        return false;
