...
// Deterministic translation failures are verdicts on the input bytes and are
// remembered. The header-only object used above never reaches translation, so
// it cannot exercise either decode or dispatchability failures.
TEST_F(HsaHotswapMemoTest, DeterministicTranslationFailuresAreRemembered) {
  ASSERT_TRUE(OnLoad(&api.table, 0, 0, nullptr));
  const std::vector<uint8_t> base = read_translation_fixture();
  ASSERT_FALSE(base.empty()) << GFX1250_B0_TO_A0_FIXTURE;
  ...
  // An all-ones opcode is rejected by the decoder. The translator contains that
  // exception as a deterministic legalization diagnostic, so it is remembered
  // just like any other verdict on the same bytes.
  rj_test_clear_retained_storage();
  ...
  EXPECT_EQ(rj_test_translation_count(), 1u) << "a decoder verdict was re-derived";
}
