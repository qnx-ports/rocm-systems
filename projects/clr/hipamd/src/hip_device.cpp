...
hipError_t hipDeviceGetLuid(char* luid, unsigned int* deviceNodeMask, hipDevice_t device) {
  ...
  *deviceNodeMask = info.luidDeviceNodeMask_;
  ...
  HIP_RETURN(hipSuccess);
#else
  // The LUID is a Windows/DXGI adapter concept and is not meaningful on other
  // platforms. Match CUDA, which returns CUDA_ERROR_NOT_SUPPORTED here and does
  // not write the output parameters.
...
#if defined(_WIN32)
  // The LUID is only meaningful on Windows; match CUDA, which reports an
  // all-zero luid and zero node mask on other platforms.
