...
NOOP_PLAYBACK_APIS: Set[str] = {
    ...
    # hipDeviceGetName / hipDeviceGetPCIBusId / hipDeviceGetLuid write into a caller-sized
    # char buffer. The generator emits `char _out{}` (1 byte) causing a stack-smash on Linux.
    # These are query-only calls with no effect on replay correctness — noop.
    "hipDeviceGetName",
    "hipDeviceGetPCIBusId",
    # hipDeviceGetLuid — output char luid[8] written into a caller-sized buffer (stale)
    "hipDeviceGetLuid",
