...
- hip-tests can be built with `-Wall -Werror` so that compiler warnings are treated as errors, controlled by the cmake flag `-DHIP_TESTS_ENABLE_WERROR`. It defaults to `ON` for Linux and `OFF` otherwise. Disable it with `-DENABLE_WERROR=OFF`. Applies to the AMD platform only.
