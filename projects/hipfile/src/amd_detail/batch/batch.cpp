...
namespace {
    ...
    bool isZeroTimeout(const struct timespec *timeout) noexcept
    {
        return timeout != nullptr && timeout->tv_sec == 0 && timeout->tv_nsec == 0;
    }

    void validateTimeout(const struct timespec *timeout)
    {
        if (timeout == nullptr) {
            return;
        }
    ...
    std::chrono::steady_clock::time_point timeoutDeadline(const struct timespec *timeout)
    {
...
BatchContext::getStatus(unsigned min_nr, unsigned *nr, hipFileIOEvents_t *iocbp, struct timespec *timeout)
{
    ...
    *nr                           = 0;
    ...
    if (num_terminal >= min_nr || num_terminal == outstanding_ops.size() || isZeroTimeout(timeout)) {
    ...
    if (timeout == nullptr) {
        ...
        status_cv.wait_until(ulock, timeoutDeadline(timeout), ready);
    }
