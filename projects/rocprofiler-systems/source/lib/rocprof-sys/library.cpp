...
// Does NOT start the worker thread - that must happen after State::Active,
// since the worker captures its delay/duration reference point (t0) at
// start() time, and starting any earlier lets init-time overhead eat into
// the window.
template <typename Clock>
std::unique_ptr<rocprofsys::control::triggers::time_window<Clock>>
make_time_window(
    rocprofsys::control::session& session, Clock& clock,
    typename rocprofsys::control::triggers::time_window<Clock>::config cfg,
    rocprofsys::control::scope event_scope = rocprofsys::control::scope::global)
{
    ...
    return std::make_unique<window_t>(session, clock, cfg, event_scope);
}
...

        {
            ...
            if(auto _trace_specs = constraint::get_trace_specs(); !_trace_specs.empty())
            {
                ...
                if(constraint::get_trace_period_clock_id() == CLOCK_PROCESS_CPUTIME_ID)
                {
                    ...
                    g_posix_trace_window =
                        make_time_window(g_control_session, *g_posix_window_clock,
                                         posix_trace_window_t::config{ _delay, _dur });
                }
                else
                {
                    g_trace_window =
                        make_time_window(g_control_session, g_trace_window_clock,
            ...
            if(const auto _samp_dur = config::get_sampling_duration(); _samp_dur > 0.0)
            {
                g_sampling_dur_window = make_time_window(
                    g_control_session, g_sampling_dur_window_clock,
                    trace_window_t::config{
                        {},
